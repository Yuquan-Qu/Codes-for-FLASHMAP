...
'''Produce Monthly FLASHMAP NetCDF Files'''
...
# Output directories
OUT_DIR_NC = '/gpfs/work2/0/einf3869/GLD360/Integrator_era/FLASHMAP/Monthly_NetCDFs'


def GLD360_vector_to_monthly_grid(month_range, resolution=resolution, IN_DIR_PAR=IN_DIR_PAR, OUT_DIR_NC=OUT_DIR_NC):

    """
    Generate monthly gridded NetCDF from the GLD360 data.
    ...
    '''
    GLD360_flash_data = GLD360_data.sort_values('time').drop_duplicates(subset=['flashId'], keep='first').copy()  # The first stroke in flash
    ...
    Three separate files are generated per month:
    ...
    '''
    flash_file_to_export = f'FLASHMAP_flash_monthly_050deg_{month_range}.nc'
    ...
    '''
    flash_file.title = f'FLASHMAP monthly 0.5° flash dataset: {month_range}'
    ...
    '''
    CG_file_to_export = f'FLASHMAP_CG_stroke_monthly_050deg_{month_range}.nc'
    # Create output directory if it does not exist
    ...
    Add global attributes
    '''
    CG_file.title = f'FLASHMAP monthly 0.5° CG stroke dataset: {month_range}'
    ...
    '''
    Initialize the IC stroke NetCDF file
    '''
    IC_file_to_export = f'FLASHMAP_IC_stroke_monthly_050deg_{month_range}.nc'
    # Create output directory if it does not exist
    ...
    Add global attributes
    '''
    IC_file.title = f'FLASHMAP monthly 0.5° IC stroke dataset: {month_range}'
    # More to be added
